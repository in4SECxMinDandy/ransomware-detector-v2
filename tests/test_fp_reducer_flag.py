...
feature 16 ``Is Known Benign Format`` already encodes the same signal,
producing a double-counted FP-reduction. The flag lets operators disable
the discount so the model alone owns that decision.
"""
...
class TestMagicBytesDiscountFlag:
    def test_default_applies_discount(self, png_file, monkeypatch):
        """Default behaviour (flag absent) ⇒ probability ×0.70."""
        monkeypatch.setattr(
    ...
    def test_helper_reads_config(self, monkeypatch):
        """``_magic_bytes_discount_enabled`` returns False iff config flag set."""
