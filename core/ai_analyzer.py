import os
import json
import logging
from typing import Dict, Any, Optional

import anthropic

from core.config_manager import config

logger = logging.getLogger(__name__)


class AIAnalyzer:
    """
    AI Analyzer using Anthropic (Claude Sonnet 4.6) API to assess ransomware
    threats and behavior alerts.
    """

    def __init__(self):
        if config.get("ai.enabled") is None:
            config.set("ai.enabled", True)
        if not config.get("ai.model"):
            config.set("ai.model", "claude-sonnet-4-6")

        self.api_key    = config.get("ai.api_key", "")
        self.model      = config.get("ai.model", "claude-sonnet-4-6")
        self.max_tokens = config.get("ai.max_tokens", 1024)
        self.temperature = config.get("ai.temperature", 0.2)
        self.enabled    = config.get("ai.enabled", True)

    def is_configured(self) -> bool:
        return self.enabled and bool(self.api_key)

    def _build_client(self) -> anthropic.Anthropic:
        return anthropic.Anthropic(api_key=self.api_key)

    def analyze_threat(self, threat_data: Dict[str, Any]) -> str:
        """
        Send threat data to Claude API for analysis and recommendations.
        """
        if not self.is_configured():
            return "AI Analysis is not configured. Please check your API key."

        prompt = f"""You are a senior cybersecurity malware analyst.
Please analyze the following potential ransomware/malware threat data.
Provide a concise summary of the risk, potential impact, and clear recommended actions for incident response.

Threat Data Context:
{json.dumps(threat_data, indent=2, default=str)}

Analysis Report:"""

        try:
            client = self._build_client()
            response = client.messages.create(
                model=self.model,
                max_tokens=self.max_tokens,
                temperature=self.temperature,
                system="You are a cybersecurity expert analyzing potential malware and ransomware threats.",
                messages=[
                    {
                        "role": "user",
                        "content": prompt,
                    }
                ],
            )

            if response.content and len(response.content) > 0:
                return response.content[0].text
            else:
                return "AI returned an empty response."

        except anthropic.APIStatusError as e:
            logger.error(f"Claude API error ({e.status_code}): {e.message}")
            return f"Claude API error ({e.status_code}): {e.message}"
        except Exception as e:
            logger.error(f"Claude AI Analysis failed: {e}")
            return f"Error analyzing threat with AI: {str(e)}"


# Singleton instance access
_ai_analyzer_instance: Optional[AIAnalyzer] = None


def get_ai_analyzer() -> AIAnalyzer:
    global _ai_analyzer_instance
    if _ai_analyzer_instance is None:
        _ai_analyzer_instance = AIAnalyzer()
    return _ai_analyzer_instance
